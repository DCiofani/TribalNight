...
- _(prossimo: **Fase 5 — cutover prod** (richiede ok): wiro `DATABASE_URL`/`AUTH_DB_URL`/`DATABASE_URL_DIRECT` + `JWT_SECRET` su Railway al Postgres dello stack (+ password `authenticator`, applica `0000/0003/0004`), flip `NEXT_PUBLIC_BACKEND=api`, verifica, spegni Kong/GoTrue/PostgREST/Realtime)_
